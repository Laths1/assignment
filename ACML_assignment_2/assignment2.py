...
class kMeans:
    def __init__(self, k, dataset, initialCentres):
        self.k = k
        self.dataset = dataset
        self.clusters = []
        for i in range(k):
            cluster = {
                "mean": initialCentres[i],
                "points": []
            }
    ...
    def train(self):
        ...
        while not converged:
            ...
            empty_clusters = [c for c in self.clusters if not c["points"]]
            if empty_clusters:
                self.clusters = [c for c in self.clusters if c["points"]]
                self.k = len(self.clusters)
                # Recalculate means of remaining clusters to ensure they’re current
                for c in self.clusters:
